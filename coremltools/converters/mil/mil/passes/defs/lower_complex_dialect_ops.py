...
def _stft(
    ...
    if normalized and normalized.val:
        ...
        real_result = mb.real_div(x=real_result, y=multiplier, before_op=before_op)
        imag_result = mb.real_div(x=imag_result, y=multiplier, before_op=before_op)
...
def _lower_complex_istft(op: Operation):
    print(f"🔍 ISTFT return_complex: {op.return_complex}")  # Debug
    print(f"🔍 ISTFT return_complex value: {op.return_complex.val if op.return_complex else None}")  # Debug
    ...
    if op.return_complex and op.return_complex.val:  # Check both existence and value
        print("🔍 Returning complex output")  # Debug
        return _wrap_complex_output(op.outputs[0], real, imag)
    else:
        print("🔍 Returning real output")  # Debug
        return real
